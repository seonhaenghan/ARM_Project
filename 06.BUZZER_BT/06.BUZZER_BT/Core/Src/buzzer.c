...
STM32에서 주파수를 만들 때 3개의 레지스터를 설정한다.
 ...
 다음과 같이 적용 하면 된다.
 - PSC : Timer Clock / 기준으로 사용할 주파수 - 1 (52.5)
 - ARR : 기준 주파수 / 실제 주파수 - 1 (400)
...
예를 들어 동작 클럭이 84Mhz이고 4Khz에 50%비율로 동작하는 PWM을 만들고 싶다면 식은 다음과 같다.
 ...
 CCRx(Capture Compare Register) : duty 비 결정 = 199
 __HAL_TIM_SET_AUTORELOAD() :  Frequency를 설정 한다.
...
void buzzer_main()
{
  ...
  while (1)
  {
    ...
    /* 음악 끝나고 3초 후 시작*/
    HAL_TIM_PWM_Stop(&htim3, TIM_CHANNEL_1) ;	// PWM STOP Func
    HAL_Delay(3000);
